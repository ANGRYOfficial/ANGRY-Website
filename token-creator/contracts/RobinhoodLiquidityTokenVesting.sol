 ...
 *
 * Supported by this Base build:
 * - Base Mainnet -> Uniswap V2
 * - Base Sepolia -> Uniswap V2
 *
...
contract AngryLiquidityTokenVesting {
    struct FeeConfig {
        uint16 holder;
        uint16 liquidity;
        uint16 project;
    ...
    constructor(
        ...
        require(
            ...
            "Vesting beneficiary required"
        );

        require(
            vesting_.amount > 0,
            "Vesting amount required"
        );

        require(
            vesting_.amount <= supply_,
            "Vesting exceeds supply"
        );

        require(
            vesting_.duration >= 30 days,
            "Vesting minimum 30 days"
        );

        require(
            vesting_.duration <= 3650 days,
            "Vesting max 10 years"
        );

        require(
            vesting_.cliff < vesting_.duration,
            "Cliff must be below duration"
        );

        require(fees_.holder <= 500, "Holder fee max 5%");
        require(fees_.liquidity <= 500, "Auto liquidity max 5%");
        require(fees_.project <= 500, "Project fee max 5%");
        require(fees_.burn <= 500, "Burn fee max 5%");

        require(
            uint256(fees_.holder) +
                uint256(fees_.liquidity) +
                uint256(fees_.project) +
                uint256(fees_.burn) <= 1000,
            "Total fee max 10%"
        );
        ...
        holderFeeBps = fees_.holder;
        liquidityFeeBps = fees_.liquidity;
        projectFeeBps = fees_.project;
        burnFeeBps = fees_.burn;
        ...
        AngryVestingVault vault_ =
            new AngryVestingVault(
                address(this),
        ...
        // Vesting principal must remain exact and must never
        // increase through holder reflection.
        _excludeFromReflection(vestingVault);
        ...
        if (
            (fees_.liquidity > 0 || fees_.project > 0) &&
            router_ != address(0)
        ) {
        ...
        // Fixed supply only:
        // the vesting allocation is moved from the deployer's
        // initial supply, never minted on top of it.
        _rawTransfer(
            msg.sender,
            vestingVault,
            vestingAmount
        ...
        internal
    {
        ...
        // The Vesting Vault receives its allocation only during
        // construction. Extra deposits are blocked so the schedule
        // can never become inconsistent with its fixed allocation.
        require(
            to != vestingVault,
            "Vesting vault deposits disabled"
        );

        // Scheduled releases must deliver the exact vested amount.
        // They bypass Holder Reflection, Auto Liquidity,
        // Project Fee and Burn.
        if (from == vestingVault) {
            _rawTransfer(
                from,
                to,
                amount
    ...
    function _routerForChain()
        ...
        returns (address)
    {
        // Robinhood Chain Mainnet - Uniswap V2 Router02
        if (
            block.chainid == 4663
        ) {
            return
        ...
        // Robinhood Chain Testnet - V2-compatible test DEX
        // Test infrastructure only; production uses Uniswap on Mainnet.
        if (
            block.chainid == 46630
        ) {
            return
                0x0Cb47D1A9Fa3028eCC12Dd4688ABdE313A7a194e;
        }
