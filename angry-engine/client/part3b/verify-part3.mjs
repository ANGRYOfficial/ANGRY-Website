...
async function waitForSignatureStatus(
  ...
  for (let attempt = 1; attempt <= 90; attempt++) {
     ...
     * bytes periodically. Same bytes => same signature.
     */
    if (raw && attempt % 5 === 0) {
...
async function sendV0ExpectedFailure(
   ...
   * the runtime so atomic rollback can be checked. Retry always resends
   * the same serialized transaction/signature.
   */
  const raw = tx.serialize();
...
async function runVerify(mainWallet, plan) {
  ...
  const successSimulation = await simulateV0(
    ...
    [tempAuthority],
    executeLookupTable,
    "success candidate min_base_out=1"
  );
