...
const tests = [
  {
    label:
      "ATTACK 14 — WRONG ENGINE AUTHORITY",

    /*
     * Still marked as signer in the instruction, but it is
     * not config.authority. Anchor has_one must reject it.
     */
    overrides: {
      authority:
        MAIN_WALLET,
    ...
    expected:
      /ConstraintHasOne|has one/i,
  },

  {
    label:
      "ATTACK 15 — WRONG ENGINE VAULT",

    /*
     * Deliberately substitute another existing ANGRY-owned
     * account. Anchor account typing / PDA constraints must
     * reject it before handler execution can use the vault.
     */
    overrides: {
      vault:
        OLD_CONFIG,
    ...
    expected:
      /AccountDiscriminatorMismatch|AccountDidNotDeserialize|ConstraintSeeds/i,
  },

  {
    label:
      "ATTACK 16 — WRONG QUOTE MINT",

    /*
     * deploy_liquidity requires the quote mint to be WSOL.
     *
     * If the project mint belongs to Token-2022, Anchor may
     * reject it at account typing before reaching the custom
     * address error. Either outcome is valid protection.
     */
    overrides: {
      quoteMint:
        baseMint,
    ...
    expected:
      /InvalidLiquidityQuoteMint|AccountOwnedByWrongProgram|AccountDidNotDeserialize|ConstraintAddress/i,
  },

  {
    label:
      "ATTACK 17 — FAKE PUMPSWAP EVENT AUTHORITY",
    ...
    expected:
      /InvalidPumpSwapEventAuthority/i,
  },

  {
    label:
      "ATTACK 18 — FAKE GLOBAL VOLUME ACCUMULATOR",
    ...
    expected:
      /InvalidPumpGlobalVolumeAccumulator/i,
  },

  {
    label:
      "ATTACK 19 — FAKE BREAKING BUYBACK FEE ATA",

    /*
     * Existing WSOL ATA, but belongs to the protocol-fee
     * recipient rather than the chosen buyback recipient.
     */
    overrides: {
      breakingFeeAta:
        protocolFeeAta,
    ...
    expected:
      /InvalidBreakingFeeRecipientAta/i,
  },

  {
    label:
      "ATTACK 20 — FAKE LIQUIDITY LP TOKEN ACCOUNT",

    /*
     * Existing token account controlled by liquidity PDA,
     * but its mint is the project token, not the LP mint.
     */
    overrides: {
      liquidityLpAta:
        liquidityBaseAta,
    ...
    expected:
      /InvalidLiquidityLpTokenAccount/i,
  },

  {
    label:
      "ATTACK 21 — FAKE COIN CREATOR VAULT ATA",

    /*
     * Existing WSOL ATA, but not the canonical ATA of the
     * creator-vault authority derived from Pool.coinCreator.
     */
    overrides: {
      creatorVaultAta:
        protocolFeeAta,
    ...
    expected:
      /InvalidPumpSwapCreatorVault/i,
  },

  {
    label:
      "ATTACK 22 — FAKE POOL QUOTE TOKEN ACCOUNT",

    /*
     * Substitute the real pool base vault in the quote-vault
     * position. Pool state binding must reject the swap.
     */
    overrides: {
      poolQuoteTokenAccount:
        poolBaseTokenAccount,
