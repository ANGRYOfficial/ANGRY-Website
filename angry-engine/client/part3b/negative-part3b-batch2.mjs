...
const tests = [
  {
    label:
      "ATTACK 5 — FAKE LIQUIDITY BASE TOKEN ACCOUNT",

    /*
     * Existing valid token account, but it is the WSOL ATA,
     * not the canonical project/base ATA.
     */
    overrides: {
      liquidityBaseAta:
        liquidityWsolAta,
    ...
    expected:
      /InvalidLiquidityBaseTokenAccount/i,
  },

  {
    label:
      "ATTACK 6 — FAKE LIQUIDITY WSOL ACCOUNT",

    /*
     * Existing valid Legacy SPL token account, but owned by
     * the protocol fee recipient rather than liquidity PDA.
     */
    overrides: {
      liquidityWsolAta:
        protocolFeeAta,
    ...
    expected:
      /InvalidLiquidityWsolAccount/i,
  },

  {
    label:
      "ATTACK 7 — FAKE LP MINT",

    /*
     * WSOL mint exists and is a valid mint account,
     * but it is not PumpSwap's Token-2022 LP mint.
     */
    overrides: {
      lpMint:
        quoteMint,
    ...
    expected:
      /InvalidLiquidityLpMint/i,
  },

  {
    label:
      "ATTACK 8 — FAKE COIN CREATOR VAULT AUTHORITY",
  ...
  {
    label:
      "ATTACK 9 — FAKE GLOBAL CONFIG",

    /*
     * poolV2 exists on-chain, avoiding AccountNotFound,
     * but it is not the canonical global_config PDA.
     */
    overrides: {
      globalConfig:
        poolV2,
    ...
    expected:
      /InvalidPumpSwapGlobalConfig/i,
  },

  {
    label:
      "ATTACK 10 — FAKE FEE CONFIG",
    ...
    expected:
      /InvalidPumpFeeConfig/i,
  },

  {
    label:
      "ATTACK 11 — FAKE PROTOCOL FEE ATA",

    /*
     * Existing WSOL ATA, but belongs to another recipient.
     */
    overrides: {
      protocolFeeAta:
        breakingFeeAta,
    ...
    expected:
      /InvalidPumpSwapProtocolFeeAta/i,
  },

  {
    label:
      "ATTACK 12 — FAKE BREAKING/BUYBACK FEE RECIPIENT",
    ...
    expected:
      /InvalidBreakingFeeRecipient/i,
  },

  {
    label:
      "ATTACK 13 — BASE MINT / PROJECT SUBSTITUTION",

    /*
     * quoteMint is a real mint account, but config.project
     * must remain the original PumpSwap base/project mint.
     */
    overrides: {
      baseMint:
        quoteMint,
