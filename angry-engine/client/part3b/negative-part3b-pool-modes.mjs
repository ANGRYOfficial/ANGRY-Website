...
async function verifyModeTarget(
  target,
  expectedMayhem,
  expectedCashback,
  label
  ...
  if (!info)
    throw new Error(`${label}: pool account missing`);

  if (!info.owner.equals(PUMPSWAP_PROGRAM_ID))
    throw new Error(`${label}: not owned by PumpSwap`);

  if (info.data.length <= 244)
    throw new Error(`${label}: pool data too short`);

  if (!info.data.subarray(0, 8).equals(POOL_DISC))
    throw new Error(`${label}: bad Pool discriminator`);
  ...
  if (
    ...
    throw new Error(
      `${label}: live flags changed from expected values`
    );
  }
}

await verifyModeTarget(
  MAYHEM_POOL,
  true,
  false,
...
const tests = [
  {
    label:
      "MODE ATTACK 1 — REAL MAYHEM POOL",
    ...
    expected:
      /UnsupportedPumpSwapPoolMode/i,
  },

  {
    label:
      "MODE ATTACK 2 — REAL CASHBACK POOL",
...
console.log(
  "\n✅ ALL UNSUPPORTED POOL-MODE TESTS REJECTED"
);
